...
$J$ can be minimized numerically by solvers like [GTSAM](http://ceres-solver.org/|Ceres]] or [[https://gtsam.org/), which will use a solver like Levenberg-Marquardt. If we wanted to solve it analytically, setting its derivative with respect to $\hat{x}$ equal to zero yields the *necessary condition*:
...
$$\hat{x}_{k+1}=\hat{x}_k+Q_{k+1}H_{\hat{x}_k}^TR^{-1}(y-h(\hat{x}_k))+Q_{k+1}Q_0^{-1}(x_0-\hat{x}_k),~Q_{k+1}=(Q_0^{-1}+H_{\hat{x}_k}^TR^{-1}H_{\hat{x}_k})^{-1}$$
